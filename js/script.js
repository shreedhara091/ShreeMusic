...
async function getSongs(folder) {
    ...
    // Fetch the folder contents from the local server.
    let a = await fetch(`http://127.0.0.1:3000/${currFolder}/`);
    let response = await a.text();

    // Convert the server response into a DOM element.
    let div = document.createElement("div");
    div.innerHTML = response;

    // Find all links and keep only MP3 files.
...
async function displayAlbum() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("Songs")) {
            // Extract the playlist folder name.
                        ...
                        <div class="play">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="38" height="38">
                                ...
                                <path
                                    ...
                                    fill="#000000" transform="translate(12 12) scale(0.65) translate(-12 -12)" />
                            </svg>
                        </div>
                        <img src="/Songs/${folder}/cover.png" alt="card-1">
    ...
    Array.from(document.getElementsByClassName("card")).forEach(e => {
        e.addEventListener("click", async item => {
            ...
            // Play the first song in the selected playlist.
            playMusic(songs[0]);
        })
    })
}
...
async function main() {
    ...
    // Update the progress bar and song time while the song is playing.
    ...
    // Allows the user to seek to a specific position in the song.
    ...
    // Controls the audio volume using the volume slider.
    document.querySelector(".range").getElementsByTagName("input")[0].addEventListener("change", (e) => {
        ...
        if (currentSong.volume > 0) {
            ...
            document.querySelector(".volume>img").src =
                document.querySelector(".volume>img").src.replace("img/volume.svg", "img/mute.svg");
        }
    })
